...
int main(int argc, char *argv[])
{
	...
	if (strcmp(argv[1], "ident") == 0) {
		...
		if ((val & 0xff) != 0xed) {
			fprintf(stderr,
				"read value has unexpected format: 0x%08x\n",
				val);
			return EXIT_FAILURE;
		}
		...
		dma_mem = mmap(NULL, EDU_DMA_BUF_SIZE, PROT_WRITE, MAP_SHARED,
			       fd, 0);
		...
		if (num_read < 0)
			goto read_fail;
		ret = ioctl(fd, EDU_IOCTL_DMA_TO_DEVICE,
		...
		dma_mem = mmap(NULL, EDU_DMA_BUF_SIZE, PROT_READ, MAP_SHARED,
			       fd, 0);
		if (dma_mem == MAP_FAILED)
			goto mmap_fail;

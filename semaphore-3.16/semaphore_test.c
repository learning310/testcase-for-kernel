...
static int rwsem_show(struct seq_file *m, void *v)
{
	...
	if (!list_empty(&rwsem.wait_list)) {
		// The first waiter
		waiter = list_entry(rwsem.wait_list.next, struct rwsem_waiter,
				    list);
			   ...
			   size_t count, loff_t *offs)
{
	...
	if (strncmp(cmd, "down_read", 9) == 0) {
		task = kthread_create(downread, cmd + 10, "%s", cmd + 10);
		if (IS_ERR(task)) {
			return -EFAULT;
		}
		wake_up_process(task);
	} else if (strncmp(cmd, "up_read", 7) == 0) {
		task = kthread_create(upread, cmd + 8, "%s", cmd + 8);
		if (IS_ERR(task)) {
			return -EFAULT;
		}
		wake_up_process(task);
	} else if (strncmp(cmd, "down_write", 10) == 0) {
		task = kthread_create(downwrite, cmd + 11, "%s", cmd + 11);
		if (IS_ERR(task)) {
			return -EFAULT;
		}
		wake_up_process(task);
	} else if (strncmp(cmd, "up_write", 8) == 0) {
		task = kthread_create(upwrite, cmd + 9, "%s", cmd + 9);
		if (IS_ERR(task)) {
			return -EFAULT;
		}
		wake_up_process(task);
